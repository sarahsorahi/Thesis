...
def main():
    ...
    cat_labels = ['Verbal\n(رفتم)', 'Possessive\n(کتابم)', 'Copular\n(خوشحالم)']
    cats_order = ['verbal', 'possessive', 'copular']
    matrix = np.zeros((3, 3))
    for i, c1 in enumerate(cats_order):
        for j, c2 in enumerate(cats_order):
            matrix[i, j] = cosine[f"{c1}_vs_{c2}"]
    ...
    print(f"  ✓ Saved cosine heatmap: {cos_path}")

    # ── Confusion matrix ──────────────────────────────────────────
    from sklearn.preprocessing import LabelEncoder as LE2
    le2 = LE2()
    le2.fit(train_df['category'].tolist() + test_df['category'].tolist())
    cm_array = np.array(svm_results["cm"])
    classes = svm_results["classes"]
